...
def is_failure_text(s: str) -> bool:
    """判断文本是否为失败/错误返回 (统一错误前缀约定).

    工具层返回约定: "[exit N] ..." (命令非零退出) / "错误: ..." /
    "文件不存在: ..." / "目录不存在: ..." — 谓词集中定义, 避免各处
    前缀判断漂移 (曾出现 note_store/hermes/talk 各自维护的漂移).
    """
    return s.startswith(("[exit", "错误", "文件不存在", "目录不存在"))
